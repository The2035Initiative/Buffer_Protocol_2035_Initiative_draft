...
```{r}
setdiff(colnames(wind_data), colnames(wind_sf))   # displays the wind_data column name wind_sf lacks
...
The issue with mapping the ejscreen data is the inclusion of US territories that are not considered in the U.S. Wind Data. Many of these territories are categorized under Region Code 9. There are a couple of ways to approach this issue. Below, we will review two strategies to removing unwanted locations to isolate our region of interest (roi).

## Isolating Region of Interest

### Strategy 1: Filtering using Location Names

The first strategy filters out those specific locations from the data without removing desired states in the Pacific region. [Region Codes](https://ncsesdata.nsf.gov/docs/location.html) reveals the various regions throughout the US that are identifiable using codes between 1:10. US Territories are categorized under the Pacific Region Code, 9. Other states like CA, OR, HA, AK, and WA are also in the Pacific Region.

```{r}
# inspecting which region codes are in our dataset
unique(ejscreen$REGION)
```

#### Identifying the Unique State Names
...
mapping_states <- (ejscreen$STATE_NAME != nonmapping_territories)

us_basemap_data <- ejscreen %>% 
  filter(STATE_NAME == mapping_states) %>% 
  group_by(STATE_NAME)
```

### Strategy 2: Create a Mask using US Country Coordinates
...
```{r}
# combine mainland states and alaska polygons into one object
mainland_alaska <- st_union(mainland_states, alaska)

# combine all polygons into a single multipolygon item
...
# outputs only T/F list
#states_matrix <- as.matrix(states_within)



#states_intersection <- st_intersection(ejscreen, states_bbox)

states_mask <- st_crop(ejscreen, states_bbox)
...
Comparing the corrected $$R^2$$ values in each of the models provided below will assist in the determination of including an additional variable to our model would be beneficial or detrimental to our Ordinary Least Sqaures (OLS) estimations.

##### Ex. 5: OVB test for legislation use estimation

```{r}
# calling an example to compare adjusted R^2 with
glimpse(legislation_est)

# model removing age 
legislation_est <- glm(legislation_use ~ status + med_inc,
                             random_sample,
                             family = 'binomial')
glimpse(legislation_est)
```
